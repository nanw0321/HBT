...
nsubpxl = 8							# number of subpixels corresponding to each real pixel/dimension
nwindow = 8							# number of windows in each dimension to prevent memory overflow
...
xdetw = np.linspace(-detsize/2-pxlsize*(1/2-1/2/nsubpxl),detsize/2+pxlsize*(1/2-1/2/nsubpxl),int(Npxl*nsubpxl)).reshape((nwindow,Npxlw))
ydetw = xdetw.copy()						# reshape for detector pixels in each window
...
taming = cuda.device_array(shape=(Natom,51,51),dtype='float64')
rsming = cuda.device_array(shape=(Natom,51,51),dtype='float64')
tamaxg = cuda.device_array(shape=(Natom,51,51),dtype='float64')
rsmaxg = cuda.device_array(shape=(Natom,51,51),dtype='float64')
...
# taxis calculation
xxmin = np.arange(51)*pxlsize/10; xxmin = xxmin-xxmin.mean()
yymin = xxmin.copy()

xxmax = np.arange(51)*pxlsize/10; xxmax = xxmax-xxmax.mean() + detsize/2
yymax = xxmax.copy()

xxming = cuda.to_device(xxmin); yyming = cuda.to_device(yymin)
xxmaxg = cuda.to_device(xxmax); yymaxg = cuda.to_device(yymax)

getaxis[(8,8),(8,8)](rsming,taming,xsg,ysg,zsg,tsg,xxming,yyming,rg[0])
getaxis[(8,8),(8,8)](rsmaxg,tamaxg,xsg,ysg,zsg,tsg,xxmaxg,yymaxg,rg[0])
tamin = taming.copy_to_host()
tamax = tamaxg.copy_to_host()

trange = tamax.max()-tamin.min() + 8*dtslist.max()
mm = int(np.round(trange*1e16)+1)
tmm = np.arange(mm)/10
taxis = tmm*1e-15+tamin.min()
taxisg = cuda.to_device(taxis)

# partial temporal calculation
...
img_window_holder = np.zeros((nwindow,nwindow,Npxlw,Npxlw))

for iwin in tqdm(np.arange(nwindow)):
	xlb = iwin*Npxlw_real
	xub = xlb + Npxlw_real
	for jwin in np.arange(nwindow):
		...
		yub = ylb + Npxlw_real
		# spatial component
		spatial[(16,16),(32,32)](rsg,Fsg,tag,xsg,ysg,zsg,ksg,tsg,phsg,xdetwg[iwin],ydetwg[jwin],rg[0])
		...
		for i in range(mm):
			getslice[(16,16),(32,32)](Es1g[i],Es2g[i],Es3g[i],Fsg,taxisg[i],T2g[i],tag,tausg,dtslistg)
		# speckle pattern in current window (sub-pixel)
		Imgwg = cuda.device_array(shape=(Npxlw,Npxlw),dtype='float64')
		get_img[(16,16),(32,32)](Imgwg,Es1g,Es2g,Es3g)
		Imgw = Imgwg.copy_to_host()
		#Imgw = np.zeros((Npxlw,Npxlw))
		#Imgw += iwin*Npxlw_real + jwin
		img_window_holder[iwin,jwin] = Imgw
